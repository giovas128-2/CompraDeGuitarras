...
public class frmlogin extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/iconcerrar.png"))); // NOI18N
    ...
    if (!usuario.equals("") && !pass.equals("")) {
        try {
            PreparedStatement ps = (PreparedStatement) cn.prepareStatement(
                "SELECT tipo_usuario FROM usuarios1 WHERE email=? AND contraseña=?"
            );
            ...
            if (rs.next()) {    
                String updateHora = "UPDATE usuarios1 SET hora_entrada=NOW() WHERE email=?";
PreparedStatement psHora = cn.prepareStatement(updateHora);
psHora.setString(1, usuario);
psHora.executeUpdate();
                String tipousuario = rs.getString("tipo_usuario");
                Sesion.emailUsuario = usuario;
                PreparedStatement psBitacora =cn.prepareStatement(
                "INSERT INTO bitacora (email, hora_entrada, estado, tipo_usuario) VALUES (?, NOW(),'CORRECTO', ?)"
                );
            psBitacora.setString(1, usuario);
            psBitacora.setString(2, tipousuario);
            psBitacora.executeUpdate();
                if(tipousuario.equalsIgnoreCase("Administrador")){
                    dispose();
                    menuadmin pa = new menuadmin(new javax.swing.JFrame(), true);
                    pa.setVisible(true);
                } else if(tipousuario.equalsIgnoreCase("Usuario")){
                   dispose(); // Cierra el login
                   Menu pg = new Menu(new javax.swing.JFrame(), true); // Este es el constructor correcto
                   pg.setVisible(true);

                }
            } else {
                PreparedStatement psBitacora =cn.prepareStatement(
                "INSERT INTO bitacora (email, hora_entrada, estado, tipo_usuario) VALUES (?, NOW(),'INCORRECTO', NULL)"
                );
                psBitacora.setString(1,usuario);
                psBitacora.executeUpdate();
